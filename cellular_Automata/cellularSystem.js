...
class Cell {
    ...
    update(mode) {
        ...
        if (mode == 1) {    // 1. Basic Life
            if (this.nCount <= 1 || this.nCount >= 4) this.nextState = 0
            if (this.nCount == 3) this.nextState = 1
        } 
        else if (mode == 2){ // 2. Crazy life 
            if (this.nCount <= 1 || this.nCount >= 4) this.nextState = 0
            if (this.nCount >= 2 && this.nCount <= 3) this.nextState = 1
        } 
        else if (mode == 3) { // 3. Dissolving Pattern
            if (this.nCount <= 1 || this.nCount >= 5) this.nextState = 0
            else if (this.nCount >= 2 && this.nCount <= 4) this.nextState = 1
        }
        else if (mode == 4) { // 4. Cloud District
            if (this.nCount == 3 || this.nCount == 4 || this.nCount == 6 || 
                this.nCount == 7 || this.nCount == 8) this.nextState = 1
        }
            ...
            if (this.nCount == 2 || this.nCount == 8) this.nextState = 1
            else this.nextState = 0
        }
        else {  // Basic Life
            if (this.nCount <= 1 || this.nCount >= 4) this.nextState = 0
        ...
        if (this.nextState == 1) this.age += 1
        else this.age = 1
...
/*
        for (let i = -1; i <= 1; i++) {
            for (let j = -1; j <= 1; j++) {
                if (i == 0 && j == 0) continue
                if (cellGrid[this.x + i][this.y + j].state == 1) alivies++
            }
        }
*/
        this.nCount = alivies
...
// Grid full of Cells
    for (let i = -oOS; i < cellCount+oOS; i++) {
        cellGrid[i] = []
        for (let j = -oOS; j < cellCount+oOS; j++) cellGrid[i][j] = new Cell(i,j)
    }

// Set States
let centered = int(cellCount/2.5)
    for(let i = centered-2; i<cellGrid.length-centered; i+=2) {
        for (let j = centered-2; j<cellGrid[i].length-centered; j+=2) cellGrid[i][j].state = 1
    }
    
    displayGrid()

console.log("cellSize:",cellSize, " rowCount:", cellGrid.length-oOS, " centered:", centered)
}
...
function draw() {
    ...
    shuffleVal = shuffleInp.value | 0.01
} // END OF DRAMA
...
// Save and Redo Drawings
function keyPressed() {
    if (keyMap[83]){
        if (drawArr.length>0) saveArr.push(drawArr)
        drawArr = []
        resetGrid()
        saveArr.forEach((row) => {row.forEach((c) => cellGrid[c[0]][c[1]].state = 1)})
        displayGrid()
        console.log(saveArr.join(","))
    } else if (keyMap[82]) {
        saveArr.pop()
        resetGrid()
        saveArr.forEach((row) => {row.forEach((c) => cellGrid[c[0]][c[1]].state = 1)})
...
function shuffleField() {
    ...
    startBtn.innerText = "Start"
    background(bgC)
    if (shuffleVal > 100) shuffleVal = 99
    cellGrid.forEach((row) => {row.forEach((c) => {
            if (Math.random() < (parseInt(shuffleVal)/100)) c.state = 1
            else c.state = 0
            c.display()
        });
    });
...
function updateGrid() {
    cellGrid.forEach((row) => {row.forEach((c) => {c.state = c.nextState});});
}
